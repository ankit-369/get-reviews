...
<div className={classname}>
    ...
    {reviews.map((data) => (
        <div key={data.id} className="mx-auto  bg-gray-800 rounded-xl shadow-md overflow-hidden  max-w-4xl mb-4 border border-gray-700">
            <div className="p-4 flex flex-col">
                <div className="flex justify-between items-start mb-4">
                    <span className="inline-flex items-center rounded-full text-sm font-medium bg-blue-100 text-blue-800"></span>
                    <button onClick={() => handle_like(data.id, data.heart)} className="p-2 rounded-full hover:bg-gray-700 transition">
                        <svg
                ...
                <p className="text-white mb-4 text-base">{data.review}</p>
    ...
    <div className="flex mb-4 space-x-2 overflow-x-auto">
        {data.images.map((image, index) => (
            <img
                key={index}
                className="max-w-sm max-h-48 object-contain rounded-lg shadow-sm"
    ...
    <p className="text-gray-500 mb-4 text-sm">No images available</p>
)}

                <div className="flex items-center mb-4">
                    <img className="w-16 h-16 rounded-full mr-2 object-cover border border-gray-600 shadow-md" src={data.photo || '/user/default.png'} alt="User photo" />
                    <div>
        ...
        //                         {data.images?.map((image, index) => (
        //                             <img key={index} className="w-20 h-20 object-cover rounded-lg mr-4" src={image ? image : "https://via.placeholder.com/500x300"} />
        //                         ))}
        //                     </div>
        //                     : 
        ...
        //                     <img className="w-10 h-10 rounded-full mr-2 object-cover" src={data.photo ? data.photo : '/user/default.png'} />
        //                     <div>
        //                         <h5 className="text-white text-sm font-semibold">{data.name}</h5>
        //                         <p className="text-gray-400 text-xs">Name</p>
        //                     </div>
