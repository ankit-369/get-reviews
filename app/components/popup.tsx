  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.data.message === 'Form submitted successfully') {
        ...
        setErrors({ general: response.data.message }); // Set the backend error
        setShowAlert(true);
      ...
      {ShowSuccessMessage ?
        ...
        <div className="bg-gradient-to-r from-blue-50 via-white to-blue-50 rounded-xl shadow-2xl w-11/12 md:w-2/3 lg:w-1/2 p-10 relative border border-blue-200">
          ...
          <div className="flex items-center justify-center mb-8">
            ...
            /> */}
            <div className="" style={{width: '130px', height: '130px', position: 'relative'}}>

            <Image 
                  src={data?.imageUrl || '/user/default.png'}
                  alt="logo"
                  fill
                  className="rounded-full border-4 border-blue-400 shadow-lg transform hover:scale-110 transition-transform"
                  style={{ objectFit: 'cover' }} // Keep object-fit styling
